...
/* 	GÖREV 1:
		cumlelereDonustur fonksiyonuna aşağıdaki yönergeleri uygulayın.
			...
			4. Oluşturulan her cümle yenlup o dizi bir dizi oluşturuinin içine aktarılacak. 
			5. Oluşturulan yeni dizi döndürülecek.
	*/
	

function cumlelereDonustur(dizi, ikinci=","){
	let yenidizi = dizi.map((sonuc)=>sonuc.join(ikinci))
	return yenidizi;
}
...
geçirmek için bir plan kurdular. Tavşan diziye önden saldıracak, kirpi ise arkalarından dolaşacak. Varsayalım ki arkadaşların planları başarılı oldu. 
Tavşanı dizinin ilk elemanına 🐇, Kirpiyi dizinin son elemanına ekleyin 🦔 
*/
//3b çözümü
...
/* 	GÖREV 4:
		...
		Bunun için emojiler adında bir nesne tanımlamışlar. Kullanıcının gönderdiği mesaj stringi içinde emojiler 
		nesnesinin anahtarı(index) bulunuyorsa, bu işareti otomatik olarak anahtara ait değerde tanımlanmış emoji ile 
		değiştirecek bir fonksiyon geliştirmek istiyorlar. ÖRNEK: Mesaj içinde ":)" sembolü bulunursa mesajı alan kişi 
		bu sembolü "🙂" olarak görecek. Burdan yola çıkarak emojileriDonustur fonksiyonuna aşağıdakileri uygulayın.
...
function emojileriDonustur(mesaj,emojiobj){
	...
	for ( let key in emojiobj) {
		...
		mesaj=mesaj.replaceAll(key, emojiobj[key]);
	}
return mesaj;
}
